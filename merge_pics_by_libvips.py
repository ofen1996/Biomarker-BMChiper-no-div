...
def generate_whole_tif(pics_dirs, save_path=None, compression='jpeg'):
    if save_path is None:
        save_path = os.path.join(os.path.split(pics_dirs)[0], os.path.split(pics_dirs)[1] + ".ome.tif")

    # 遍历目录，根据海德星格式查询shape，文件名等参数, 图像迭代器
    (tiles_down, tiles_across), (img_width, img_height) = HDX_image_prepare(pics_dirs)

    t1 = time.time()
    tiles = [pyvips.Image.new_from_file(
        os.path.join(pics_dirs, f"IMG{str(y).zfill(3)}x{str(x).zfill(3)}.tif"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_dir', type=str, help="all img dir path")
