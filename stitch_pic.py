...
def match_pic_row(img_before, img):
    ...
    min_max_loc = cv2.minMaxLoc(result)
    # print(min_max_loc)
    new_shift_y_x = np.asarray(min_max_loc[-2][::-1]) - (int(conf.x_range * 0.7), int(conf.overlap_x + conf.x_range))
    if min_max_loc[0] < 1e-04:  # 低纹理信息，不做识别，取初始位置
        new_shift_y_x = [0, -conf.overlap_x]
    print("new_shift_y_x:{}".format(new_shift_y_x), "value:{}".format(min_max_loc[0]))

    return [new_shift_y_x[0], img_before_gray.shape[1] + new_shift_y_x[1]], min_max_loc[0]
...
def match_pic_column(img_before, img):
    ...
    template_part = img_before_gray[-int(conf.overlap_y + conf.y_range):, :img_before_gray.shape[1]]
    match_part = img_gray[:int(conf.overlap_y - conf.y_range), int(conf.y_range * 0.7): -int(conf.y_range * 0.7)]
    result = cv2.matchTemplate(template_part, match_part, cv2.TM_SQDIFF_NORMED)
    min_max_loc = cv2.minMaxLoc(result)
    print(min_max_loc)
    new_shift_y_x = np.asarray(min_max_loc[-2][::-1], dtype=int) - (int(conf.overlap_y + conf.y_range), int(conf.y_range * 0.7))
    print("new_shift_y_x:{}".format(new_shift_y_x), "value:{}".format(min_max_loc[0]))
    if min_max_loc[0] < 1e-04:  # 低纹理信息，不做识别，取初始位置
        new_shift_y_x = [-conf.overlap_y, 0]
...
def stitch_from_pic(pic_dir, fov_shape, save_dir, start_pic_index=(0, 0), stitch_channel=None):
    ...
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
    try:
        tifffile.imwrite(os.path.join(save_dir, 'stitch-img.tif'), whole_img, compression="jpeg")
    ...
    level_2_path = os.path.join(save_dir, 'stitch-level-{}.tif'.format(conf.resize_level))
    level_2_img = whole_img[::conf.resize_level, ::conf.resize_level, :]
    tifffile.imwrite(level_2_path, level_2_img, compression="jpeg")

    return level_2_img, all_loc
...
class StitchImg:
    ...
    def cut_and_stitch(self):
        ...
        # 透视变换
        small_img_chip = cv2.warpPerspective(small_img, M, (img_w, img_h))
        print(f"reg_box={tmp_corners}, desc_box={desc_box}")
        tifffile.imwrite(os.path.join(self.img_path, 'level-2-chip.tif'), small_img_chip, compression="jpeg")
...
if __name__ == '__main__':
    ...
    #     (12, 13),
    #     r"E:\LQGCS\20221128-BI29BN02F2-A4-GZ-LQG-20X-Img",
    #     start_pic_index=(0, 0))
